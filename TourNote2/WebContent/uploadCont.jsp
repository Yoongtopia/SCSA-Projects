	...
	$(function(){
		...
		$("#mainCont").dialog({
			...
			buttons: {
					...
					'취소': function(){
						var href="${pageContext.request.contextPath}/upload_delete.do";
...
/* 		// upload type에 따라 title을 다르게 주기
		...
		if ("${uploadType}"=="photoUpload"){
			$("#uploadForm").attr("action","${pageContext.request.contextPath}/uploadTransportation_view_byPhoto.do");
/* 			$("#uploadForm").attr("action","${pageContext.request.contextPath}/uploadTransportation_view_byPhoto.do?uploadType=${uploadType}"); */
		} else {
			$("#uploadForm").attr("action","${pageContext.request.contextPath}/uploadTransportation_view.do");
/* 			$("#uploadForm").attr("action","${pageContext.request.contextPath}/uploadTransportation_view.do?uploadType=${uploadType}&latitude=${latitude}&longitude=${longitude}"); */
		...
		if (photoList != null && photoList.size() !=0){
			...
			var deleteBtn=$("<input/>").attr("type","button").attr("id",fileName).attr("value","x")
										...
										.click(function(){
											var id=$(this).attr("id");
											$.get("${pageContext.request.contextPath}/delete_image.do",
													{"photoName" : photoName});
											$("#Div"+id).remove();
										});
...
/* 		var file = document.uploadForm.image; */
		...
		var charCheck = /[~!@\#$%<>^&*\()\-=+\']/gi;
		if(koCheck.test(fileName) || charCheck.test(fileName)){
			alert("파일명은 한글, 특수문자를 제외한 영문, 숫자로만 설정해주세요.");
			$(".image:last-child").remove();
			addInputFile();
		} else {
			// 뒤로 가기 했다 온 경우, 이미 저장된 db에 파일 있는지 확인
			$.get("${pageContext.request.contextPath}/photo_check.do",
...
/* 	        	var id="file"+(index-1); */
	...
	function formCheck(){
		if ($("#title").val().length >100){
			alert("제목이 제한된 글자수를 초과하였습니다.");
			return false
		}
		
		if ($("#content").val().length >4000){
			alert("내용이 제한된 글자수를 초과하였습니다.");
			return false
		}
...
<div id="mainCont" title="현재 위치로 올리기">
	...
	<form name="uploadForm" id="uploadForm" method="post" enctype="multipart/form-data" onsubmit="return formCheck()">
	<div class="modal-body" id="mainDiv" >
		<div>
			<img id="progress" src="${pageContext.request.contextPath}/images/upload/progress1.png">
		</div>
	
		<label id="titleLable" for="title">Title</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<input type="text" name="title" id="title" size="67"><br/>
		<label id="contentLabel" for="content">Content</label><br/>
		<textarea rows="4" cols="74" name="content" id="content"></textarea><br/>
		
		<div id="imageDiv">
		<span id="photoSpan">Photo</span>
<!-- 		<button id="uploadFile">ADD PHOTO</button> -->
