...
<body class="container-blue2">
	...
	<table id="scrap">
		<tr>
			<th width="150px">작성자</td>
			<th width="300px">제목</td>
			<th width="50px">추천수</td>
			<th width="50px">삭제</td>
		</tr>
		<c:choose>
			<c:when test="${not empty scrap}">
				<c:forEach items="${scrap}" var="scrap">
					<tr>
						<td>${scrap.recordWriter}</td>
						<td><a href="${pageContext.request.contextPath }/selectRecord.do?recordNum=${scrap.recordNum}">${scrap.recordTitle}</a></td>
						<td>${scrap.recommend}</td>
						<td><a href="delete_scrap.do?&recordNum=${scrap.recordNum}">
 						<input type="button" name="deleteScrap" value="DEL"/></a></td>
					</tr>
				</c:forEach>
			</c:when>
		</c:choose>
		<tr>
			<td colspan="4"><a href="${pageContext.request.contextPath}/goMyPage.do"><button>마이페이지</button></a></td>
		</tr>
	</table>
