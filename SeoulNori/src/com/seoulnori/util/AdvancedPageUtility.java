...
public class AdvancedPageUtility {
	...
	public AdvancedPageUtility(int displayRowCount, int totalRowCount,
		...
		this.imagePath = imagePath+"button/";
...
//        currentPageCount: 6인 경우  4 5 6 등에서 표시할 첫 페이지 (6)
        ...
        if(firstPageCount>pagePerCount){ 
            sb.append(" <a href='javascript:pagelist("+1+")'><img src=\""+imagePath+"btn_first.png\" border='0'  hspace='3' align='absmiddle'></a>&nbsp;&nbsp;");
    	// 보여주려는 페이지 그룹의 첫페이지번호가
    	// 첫페이지 그룹 목록이면  이전페이지 단위 링크 걸지 않는다.
        }else{  
            sb.append("<img src=\""+imagePath+"btn_first.png\" border='0'  align=absmiddle>&nbsp;&nbsp;");
        }

        //  현페이지가 속한 페이지그룹의 첫페이지가 이전페이지보다 크다면 
        // 이전 페이지그룹으로 링크를 걸고
        if(firstPageCount > beforePage){
        	sb.append("<a href='javascript:pagelist("+ beforePage+")'><img src=\""+imagePath+"btn_prev.png\" border='0' hspace='3' align=absmiddle></a>&nbsp;&nbsp;");
        //  그렇지 않다면 이전 페이지그룹으로 링크를 걸지 않는다.
        }else{
        	sb.append("<img src=\""+imagePath+"btn_prev.png\" border='0'   align=absmiddle>&nbsp;&nbsp;");
        }
        ...
        if(lastPageCount < nextPage){
        	sb.append("&nbsp;&nbsp;<a href='javascript:pagelist("+ (nextPage)+ ")'><img src=\""+imagePath+"btn_next.png\" border='0' hspace='3' align=absmiddle></a>");
        //  그렇지 않다면 다음 페이지그룹으로 링크를 걸지 않는다.        	
        }else{
        	sb.append("&nbsp;&nbsp;<img src=\""+imagePath+"btn_next.png\" border='0' hspace='3' align=absmiddle>");
        }
        
        //맨 끝 페이지  link
		// 마지막 페이지 그룹목록이 아니면 다음 페이지 그룹으로 링크를 걸고
        if(lastPageCount< totalPageCount){
            sb.append("&nbsp;&nbsp;<a href='javascript:pagelist("+ (totalPageCount)+")'><img src=\""+imagePath+"btn_end.png\" border='0' align=absmiddle></a>");
   		// 마지막 페이지 그룹목록이면 링크 걸지 않는다.	            
        }else{
            sb.append("&nbsp;&nbsp;<img src=\""+imagePath+"btn_end.png\" border='0' align=absmiddle>");
        }
