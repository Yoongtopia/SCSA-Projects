...
public class Twitter {
	...
	private String twitter_consumer_key = "2mgRsXI5hkd8RLBYJfY4rkDvE";
	private String twitter_consumer_secret = "uAkwx3rErOFu627n1AM7a3SR2MSwqesSWgkBh6hxV8A00bRNq1";	
	...
	public JSONObject searchTweets(String q, int count, String result_type, String access_token, String access_token_secret){
JSONObject jsonresponse = new JSONObject();
		...
		String parameter_string = "count="+count+"&lang=ko&oauth_consumer_key=" + twitter_consumer_key + "&oauth_nonce=" + oauth_nonce + "&oauth_signature_method=" + oauth_signature_method + 
			"&oauth_timestamp=" + oauth_timestamp + "&oauth_token=" + encode(oauth_token) + "&oauth_version=1.0&q=" + encode(q) + "&result_type="+result_type;	
		System.out.println("parameter_string=" + parameter_string);
		...
		System.out.println("authorization_header_string=" + authorization_header_string);
		 ...
		 try {
			 try {
				 ...
				 BasicHttpEntityEnclosingRequest request2 = new BasicHttpEntityEnclosingRequest("GET", twitter_endpoint_path + "?count="+count+"&lang=ko&q=" + encode(q)+"&result_type="+result_type	);
				 request2.setParams(params);
				 ...
				 else
				 {
					 ...
					 {
						 jsonresponse.put("twitter_oa", jo); // this is the full result object from Twitter
					 }
	...
	public JSONObject countTweets(String q, String until, String result_type, String access_token, String access_token_secret)
	{
		JSONObject jsonresponse = new JSONObject();
		...
		// the parameter string must be in alphabetical order
		// this time, I add 3 extra params to the request, "lang", "result_type" and "q".
		String parameter_string = "count=100&lang=ko&oauth_consumer_key=" + twitter_consumer_key + "&oauth_nonce=" + oauth_nonce + "&oauth_signature_method=" + oauth_signature_method + 
			"&oauth_timestamp=" + oauth_timestamp + "&oauth_token=" + encode(oauth_token) + "&oauth_version=1.0&q=" + encode(q) + "&result_type="+result_type+"&until="+until;	
		System.out.println("parameter_string=" + parameter_string);
		String twitter_endpoint = "https://api.twitter.com/1.1/search/tweets.json";
		String twitter_endpoint_host = "api.twitter.com";
		String twitter_endpoint_path = "/1.1/search/tweets.json";
		...
		System.out.println("authorization_header_string=" + authorization_header_string);
		 ...
		 try {
			 try {
				 ...
				 // the following line adds 3 params to the request just as the parameter string did above. They must match up or the request will fail.
				 BasicHttpEntityEnclosingRequest request2 = new BasicHttpEntityEnclosingRequest("GET", twitter_endpoint_path + "?count=100&lang=ko&q=" + encode(q)+"&result_type="+result_type+"&until="+until);
				 request2.setParams(params);
				 request2.addHeader("Authorization", authorization_header_string); // always add the Authorization header
				 httpexecutor.preProcess(request2, httpproc, context);
				 HttpResponse response2 = httpexecutor.execute(request2, conn, context);
