...
public interface StampDAO {
	...
	boolean deleteStamp(String stampNum, String userId, String eventId);
	
	


	//관리자 및 사용자의 스탬프 상세보기용
	Stamp selectStampByUserIdStampNumEventId(String stampNum, String userId, String eventId);
	
	//사용자의 회원 아이디별 스탬프 리스트 보기
	//스탬프 상태가 '1(true)'인 것만 보여줌
	List<Stamp> selectStampListByUserId(String userId);
		
	//관리자의 전체 스탬프 리스트 보기
	//스탬프 상태에 상관없이 모든 stamp를 보여줌
	List<Stamp> selectStampListForAdmin();
	
	//스탬프 명예의 전당에 뿌리는 스탬프 리스트
	//아이디별로 스탬프 상태가 '1'이고 스탬프번호가 '삭제용스탬프'가 아닌 스탬프만 계산하여 
	//갯수가 많은 순서대로 보유한 스탬프 갯수와 member정보를 뿌린다.
	List<Member> selectStampListForUser();
	
	//사용자의 대표스탬프 설정
	boolean makeMainStamp(String stampNum, String userId, String eventId);
	
	//특정 카테고리에 해당하는 스탬프를 몇 개 보유하고 있는지 체크
	Integer countStampCategory(String userId, String stampNum);
	
	//관리자 스탬프 리스트 페이징
	List<Stamp> selectStampListForAdminWithPage(int startNo, int endNo);
	
	//회원 스탬프 리스트 페이징
	List<Stamp> selectStampListByUserIdWithPage(String userId, int startNo, int endNo);
