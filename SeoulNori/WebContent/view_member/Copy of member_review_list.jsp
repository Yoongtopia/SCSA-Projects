...
<title>Insert title here</title>
 <script type="text/javascript">
		 ...
		 function updateReview(count){
			 $("#reviewTable").hide();
			 $("#num").val(count);
			 $("#reviewUpdateCnt").val($("#"+count).text());
			
			
			if($("#starPoint"+count).val()==5.0 ){
				$("input:radio(0)").attr("checked","checked");
			}
			if($("#starPoint"+count).val()==4.0 ){
				$("input:radio(1)").attr("checked","checked");
			}
			if($("#starPoint"+count).val()==3.0 ){
				$("input:radio(2)").attr("checked","checked");
			}
			if($("#starPoint"+count).val()==2.0){
				$("input:radio(3)").attr("checked","checked");
			}
			if($("#starPoint"+count).val()==1.0 ){
				$("input:radio(4)").attr("checked","checked");
			}
		 
			if($("#reviewUpdateImg").val()==null){
				$("#reviewUpdateImg").val($("#reveiwImage"+count));
			}	
...
<body>
	...
	<c:when test="${not empty reviewList }">		
		<table id="reviewTable" class="table table-hover" style="table-layout:fixed; width: 800px;">
		  <tr>
		  ...
		  <tr onmouseenter="mouseEnter('${review.reviewNum}')" onmouseleave="mouseLeave('${review.reviewNum}')" style="cursor:hand" >
			...
			<td style="vertical-align: top;"><a href="${pageContext.request.contextPath }/event_detail.do?eventId=${reivew.eventId}">${review.eventTitle }</a></td>
			<td id="${review.reviewNum}" style="display:block; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${review.reviewContent }
	    ...
	    <div style="display:none;" id="reviewUpdateFormTable" >
		  <form name="reviewUpdateForm" id="reviewUpdateForm" enctype="multipart/form-data" 
			...
			<div class="row" style="padding: 20px;">
				<font color="gray" style="text-align:left;"><span class="glyphicon glyphicon-edit"></span>
